...
"""
    ...
    def find_student_id(self, name):
        for student in self.total_list:
            if name in student:
                return student[0]
        print(f"{RED}[-]未找到该名字：{name}{RESET}")
        exit(-1)
    ...
    def add_to_list(self, record, flag):
        ...
        for item in records:
            ...
            if parts[0][0].isdigit():
                thread = self.MyThread(self.find_student_id, (parts[1],))
                thread.start()
                thread.join()
                if flag == 1:
                ...
                thread = self.MyThread(self.find_student_id, (parts[0],))
                thread.start()
                thread.join()
                if flag == 1:
    ...
    def process_data_file(self):
        try:
            print("[+]获取总名单")
            for row in self.total_sheet.iter_rows(min_row=4, max_row=self.total_list_max_row, min_col=2, max_col=3):
                if str(row[0].value).isdigit():
                    self.total_list.append([row[0].value, row[1].value])
